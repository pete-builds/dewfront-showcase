...
- **Added to the home screen, it showed a generic letter.** iOS ignores an SVG `rel=icon` for the home screen entirely and the app shipped nothing else, so Safari fell back to a letterform. There is an `apple-touch-icon` now, a web manifest, and the meta tags that put the right name under the icon and launch it without browser chrome. The icon is the drop alone, white on the accent, full bleed: a home screen icon renders at about 60 points, a wordmark set inside that is unreadable, and iOS prints the name underneath anyway. Not transparent, because iOS composites transparency onto black and the icon sits on the reader's own wallpaper. Not pre-rounded, because iOS applies its own superellipse and Android its own shape. `npm run icons` generates the four PNGs by reading the path out of the favicon, so the drop still has exactly one definition in the repository and nobody has to redraw a set by hand when it moves.
- **And then the manifest shipped as a binary blob.** `.webmanifest` is not in nginx's mime.types, so it went out as `application/octet-stream`, which this server's own `nosniff` header then forbids the browser from second-guessing. Nothing broke loudly; only the installed-app metadata quietly did not work. Caught by a header check against the live site after the deploy, not by the end to end test, which runs against a preview server that already gets it right.
- **The forecaster's sentence lost its period label.** The line read "This Afternoon, National Weather Service" underneath the forecaster's own words. The outlook is always the period happening now, so the name is always "Today", "This Afternoon" or "Tonight", which restates the screen it sits on, and the sentence above already carries the timing that matters. The source stays, because the source is the half a reader cannot infer.
- **An expanded day opens at 7 AM.** It listed 12 AM through 9 PM: it began in the previous forecast day's small hours and ran out before the night the row is about. It now shows the hours the row summarises, 7 AM through 4 AM on the three hour step, and the dew point range beneath it is measured over the same window.
...
- **The same fault, one panel over: a 123°F heat alert built from a single hour.** Miami was shown a DANGER heat warning reading 123°F. The arithmetic was right and the input was not: Open-Meteo reported an 87°F dew point at 14:00 with 74°F and 76°F either side, an eleven degree round trip the atmosphere does not make, and 90°F at the 90 percent humidity that implies really is a heat index of 123.5 where the neighbouring hours are 98. The detector took the highest heat index anywhere in the rest of the day, so one bad cell set the severity for an afternoon. It now reports the level a spell HOLDS rather than its crest, computed as the best three hour window's minimum, so an hour can only lift the answer as far as its neighbours carry it. Requiring merely that the hour sit inside a run above the threshold was tried first and does not work, because the spike is above the threshold and simply joins the run it was meant to be excluded from. The current reading stays exempt: it is a reading rather than a forecast hour and it is what the hero prints as "feels like", so a strip refusing to call 106°F dangerous until it had lasted three hours would contradict the top of its own page. Re-sampled after the change: Phoenix and New Iberia still say danger on the current reading, Miami reports the 96°F it holds where it had said 102°F peak. The old test had pinned the bug, asserting that one 92°F hour among hours at 80°F fired the alert; it describes a spell now, with the Miami case beside it and a control holding that same spike value all afternoon so a detector that never escalated could not pass by doing nothing.

- **The colour key existed from the first release and nobody could reach it.** Asked what the coloured lines on the seven day rows meant, by the person who drew them. The full dew point ramp, six bands with their printed bounds, had shipped since day one on the Welcome screen, which is by definition the one screen a reader with a saved location never loads again. Every colour in the app comes off that ramp: the hero chip, the day and night chips, and the fill of every range bar in both forecast lists. None of the screens that use it said so. The ramp is a component now, rendered under the rows it explains as well as on first run, behind a disclosure because six rows of legend pushing the forecast off a phone every visit trades one irritation for a worse one. The bar also carries two independent variables and announced neither: offset and length are the day's temperature range on one scale across the week, the fill is a two stop gradient running the overnight band into the daytime one. The ten day screen had stated that in its header for weeks; the Now screen, which is denser and is where the question was asked, never had.

- **The air quality tile said what it thought and not what it measured.** The Now screen's grid printed the composite index and its EPA band and stopped, which is a number between 0 and 500 with no unit and no referent followed by the same verdict twice. The driver was already in hand: the per pollutant sub indices arrive on the same request at no extra cost and the Details panel has named the driver since September. The tile throws them away no longer. It is the half that can change a decision, which is the test for anything on this screen, because ozone peaking on a hot still afternoon is a reason to move a run to the morning and PM2.5 on a smoke day is a reason to shut the windows the card above it just recommended opening. When no pollutant can be named honestly, which happens when the upstream stops reporting sub indices or when SO2 or CO is the driver, it falls back to the bare verdict rather than printing a dangling separator.

- **"Opens after 2 AM" should say "Open after 2am", and both halves of that were wrong.** The line describes the state the windows are in after that hour and it sits in a switch whose other branches already read "Open all night" and "Open for now", so it was the only one written as an event: one card answering one question in two grammars depending on the forecast. And "2 AM" is a column heading spliced into a sentence, which is literally what it was, borrowed from the formatter built for chart axes and strip cells where the space keeps a row of them scannable. That formatter keeps its sixty call sites. Finished sentences get a second one, and the two exist on purpose rather than one being bent to serve the other.
